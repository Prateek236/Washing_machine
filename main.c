 ...
 * Author: prate
 *
 * Created on 15 March, 2024, 2:08 AM
 */
...
void init_config(void){
    //initialise the Digital Key Pad
    init_digital_keypad();
    
    //initialise the clcd
    init_clcd();
    
    //config RC2 as th output pin
    FAN_DDR = 0;
...
unsigned char *washing_program[]={"Daily","Heavy","Delicates","Whites","Stainwash","Ecocottons","Woolens","Beadsheet","Rinse + dry", "Dry only","Wash only","Aqua store"};
...
void main(void) {
    ...
    //keep waiting till the switch is pressed
    ...
    while(1){
        ...
        for (unsigned int wait = 3000; wait--;);
        //if switch 4 is pressed for long time the screen is washing mode
        if(key==LSW4 && operation_mode ==WASHING_PROGRAM_DISPLAY){
            
            operation_mode = WATER_LEVEL;
            reset_mode= WATER_LEVEL_RESET;
        }
        //if switch 4 is pressed for long time,  operation mode is water level screen or not
        else if(key==LSW4 && operation_mode ==WATER_LEVEL)
        {
            
            set_time();
            //change operation mode to start stop screen
...
void water_level_display (unsigned char key){
    ...
    //print the option based on switch press
...
void run_program(unsigned char key)
{
    ...
    if (program_no <= 7)
    {
        if (total_time >= (time - wash_time))  // ex - 100-42
        {
    ...
    else if (program_no == 8) //rinse and dry // 40% rinse // 60% wash
    {
...
void door_status_check(void)
{
    if (RB0 == 0) // if door is open
    {
        ...
        clcd_print("Pease Close" ,LINE3(0));
        while (RB0 == 0);
